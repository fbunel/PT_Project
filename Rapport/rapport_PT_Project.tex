...
\textbf{Résumé :} 
Dans ce rapport nous présentons les résultats d'une étude numérique que nous avons réalisée sur la transition de phase nématique-isotrope dans un cristal-liquide. Le modèle numérique que nous avons codé, basé sur le modèle de Lebwohl-Lasher et réalisé via une méthode de Monte-Carlo, nous a permis dans un premier temps d'observer la transition nématique-isotrope et d'obtenir avec précision la température de transition, en accord avec la littérature. Nous avons ensuite repris et amélioré notre modèle numérique pour pouvoir étudier l'influence d'un champ électrique sur cette transition. Finalement, ce dernier modèle nous à aussi permis de mettre en évidence la transition de Fréedericksz qui est au cœur de la technologie des écrans à cristaux liquides.
\vspace{0.3cm}
