...
def figure(fichier1, fichier2, skiprow):
    
    #données    
    (temperature, energie, deltaEnergie, order, deltaOrder) = donnees(
        fichier1, skiprow)

    (temperature2, energie2, deltaEnergie2, order2, deltaOrder2) = donnees(
        fichier2, skiprow)  
    ...
    ax1.plot(temperature2, order2,
        color = 'r')
    ax2.plot(temperature2, energie2, 
        color = 'y')
    ...
    plt.savefig("{}_figure.pdf".format(fichier1), bbox_inches='tight')
    plt.cla()

figure('global_study.save','local_study.save',3)